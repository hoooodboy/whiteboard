...
export class FileManager {
   ...
   * @returns
   */
  forceSaveFiles = async ({ files }: { files: BinaryFiles }) => {
    const addedFiles: Map<FileId, BinaryFileData> = new Map();

    const fileIds = Object.keys(files);
    for (const fileId of fileIds) {
      if (!this.isFileHandled(fileId as FileId)) {
        addedFiles.set(fileId as FileId, files[fileId]);
        this.savingFiles.set(fileId as FileId, true);
      }
    }
